//! Vendored + patched copy of `tree-sitter-highlight` 0.26.5 (MIT, see
//! `LICENSE`), modified for incremental ROOT-tree reuse.
//!
//! Only the root language layer's parse may reuse the previously-parsed tree
//! (via [`Highlighter::highlight_incremental`]); injection/locals layers and
//! capture precedence are preserved byte-for-byte against upstream. The
//! C-ABI binding (`c_lib.rs`) and the upstream README doc-include are dropped.
//!
//! The vendored algorithm bodies are otherwise byte-identical to upstream; the
//! only logic edits are the documented root-tree incremental plumbing.

// Vendored crate: relax the workspace's strict lints rather than editing the
// vendored logic. The upstream code relies on `unsafe`, raw pointers, and
// transmutes, and is not written to the workspace clippy profile.
#![allow(unsafe_code)]
#![allow(clippy::all)]
...
use thiserror::Error;
use tree_sitter::{
    InputEdit, Language, LossyUtf8, Node, ParseOptions, Parser, Point, Query, QueryCapture,
...
pub struct Highlighter {
    ...
    // PATCH (incremental root-tree reuse): the most recently parsed ROOT-layer
    // syntax tree, retained so the next `highlight_incremental` call can reuse
    // it as the old tree for the root parse. `None` means "full reparse next".
    prev_root: Option<Tree>,
}
...
impl Highlighter {
    ...
    pub fn new() -> Self {
        Self {
            parser: Parser::new(),
            cursors: Vec::new(),
            // PATCH: no retained root tree yet -> first call is a full reparse.
            prev_root: None,
        }
    }

    /// PATCH (incremental root-tree reuse): discard the retained root tree so
    /// the next [`Self::highlight_incremental`] call performs a full reparse.
    /// Call this when the document is replaced wholesale or the language
    /// changes, since a stale tree from a different document/grammar must not
    /// be fed to the parser.
    pub fn reset(&mut self) {
        self.prev_root = None;
    }

    /// PATCH (incremental root-tree reuse): like [`Self::highlight`], but reuses
    /// the previously-parsed ROOT-layer tree when possible.
    ///
    /// Semantics:
    /// * If a previous root tree is retained AND `edit` is `Some`, the retained
    ///   tree is `edit`ed and passed as the old tree to the ROOT parse only,
    ///   enabling incremental reparse of the root layer.
    /// * Otherwise the root layer is parsed from scratch (old tree `None`).
    /// * Injection layers are ALWAYS parsed from scratch (old tree `None`),
    ///   identical to upstream.
    /// * After the root parse, a clone of the new root tree (a cheap refcount
    ///   bump) is retained for the next call.
    ///
    /// `tree-sitter` guarantees that an incrementally-parsed tree is
    /// structurally identical to a full reparse of the same final text, so the
    /// emitted event stream is identical to the full-reparse path.
    pub fn highlight_incremental<'a>(
        ...
        // Decide the old tree for the ROOT parse only. We must satisfy the
        // borrow checker: the returned iterator borrows `&mut self`, yet we also
        // need to stash the freshly-parsed root tree into `self.prev_root`.
        //
        // Resolution: build the old tree from the retained tree BEFORE handing
        // `&mut self` to the layer constructor, and capture a clone of the new
        // root tree via an out-slot during construction; only after the layers
        // are built do we move the captured clone into `self.prev_root`. After
        // that move, `self` is free to be borrowed by the iterator.
        // Apply all accumulated edits (one `update` may cover several buffer
        // mutations) to the retained tree before reusing it. An empty slice or a
        // missing retained tree forces a full reparse; taking above also drops
        // any stale tree so we never reuse it without accompanying edits.
        let old_root: Option<Tree> = match self.prev_root.take() {
        ...
        let mut captured_root: Option<Tree> = None;
        let layers = HighlightIterLayer::new(
            ...
            old_root.as_ref(),
            Some(&mut captured_root),
        )?;
        // Retain the new root tree for the next incremental call.
        self.prev_root = captured_root;
    ...
    pub fn highlight<'a>(
        ...
        let layers = HighlightIterLayer::new(
            ...
            // PATCH: `highlight` stays a pure full-reparse path (no old tree, no
            // capture). Incremental reuse is exclusively via
...
impl<'a> HighlightIterLayer<'a> {
    ...
    fn new<F: FnMut(&str) -> Option<&'a HighlightConfiguration> + 'a>(
        ...
        // PATCH (incremental root-tree reuse): old tree for the ROOT parse only.
        // The root caller passes the edited previous tree here; the recursive
        // injection caller passes `None`. Consumed (taken) after the first parse
        // so combined injections within this same call still reparse fully.
        old_tree: Option<&Tree>,
        // PATCH: out-slot that receives a clone of the freshly-parsed ROOT tree
        // (the first parse in this call). The root caller passes `Some(&mut _)`;
        // the recursive injection caller passes `None`.
        mut root_tree_out: Option<&mut Option<Tree>>,
    ) -> Result<Vec<Self>, Error> {
        let mut result = Vec::with_capacity(1);
        let mut queue = Vec::new();
        // PATCH: the old tree applies to the ROOT parse only (the first loop
        // iteration). `.take()` ensures combined-injection parses in later
        // iterations pass `None`, matching upstream behavior.
            ...
            if highlighter.parser.set_included_ranges(&ranges).is_ok() {
                ...
                let tree = highlighter
                    ...
                    .parse_with_options(
                        ...
                        // PATCH: reuse the (edited) old tree for the ROOT parse
                        // only; `.take()` makes subsequent parses pass `None`.
                        old_tree.take(),
                ...
                // PATCH: capture a clone of the freshly-parsed ROOT tree (only
                // the first parse populates the out-slot; a clone is a cheap
                // refcount bump). Later combined-injection parses leave it.
                if let Some(out) = root_tree_out.as_deref_mut() {
                    if out.is_none() {
                        *out = Some(tree.clone());
                    }
                }
                    ...
                    if let Some(config) = (self.injection_callback)(language_name) {
                        ...
                        if !ranges.is_empty() {
                            match HighlightIterLayer::new(
                                ...
                                ranges,
                                // PATCH: injection layers always reparse from
                                // scratch and never capture a root tree.
                                None,
                                None,
                            ) {
                                Ok(layers) => {
...
// ===========================================================================
// PATCH (incremental root-tree reuse): proof tests.
//
// These assert that `highlight_incremental` (reusing the previous root tree
// through an `InputEdit`) yields a byte-for-byte identical event stream to a
// full reparse of the same final text. tree-sitter guarantees an
// incrementally-parsed tree is structurally identical to a full reparse, so
// any difference would be a bug in the incremental plumbing, not the grammar.
// ===========================================================================
#[cfg(test)]
mod incremental_tests {
    use super::{Highlight, HighlightConfiguration, HighlightEvent, Highlighter};
    use tree_sitter::{InputEdit, Point};

    // A small, fixed set of highlight names; the exact set is irrelevant to the
    // equality property — both paths use the same configuration.
    ...
    fn build_config() -> HighlightConfiguration {
        ...
        .expect("valid rust highlight configuration");
        config.configure(HIGHLIGHT_NAMES);
        config
    }

    // Compute the tree-sitter `Point` (row, byte-column) for a given byte
    // offset into `src`. tree-sitter columns are byte offsets within the row.
    ...
    // A comparable, fully-ordered encoding of a `HighlightEvent` (which is not
    // `PartialEq` upstream). Mapping per spec:
    //   Source{start,end}        -> (0, start, end)
    //   HighlightStart(Highlight(h)) -> (1, h, 0)
    //   HighlightEnd             -> (2, 0, 0)
    fn encode(ev: &HighlightEvent) -> (u8, usize, usize) {
        match *ev {
            HighlightEvent::Source { start, end } => (0, start, end),
            HighlightEvent::HighlightStart(Highlight(h)) => (1, h, 0),
            HighlightEvent::HighlightEnd => (2, 0, 0),
        }
    }

    fn full_events(config: &HighlightConfiguration, source: &[u8]) -> Vec<(u8, usize, usize)> {
        let mut hl = Highlighter::new();
        hl.highlight(config, source, None, |_| None)
            .expect("full highlight starts")
            .map(|e| encode(&e.expect("full highlight event")))
            .collect()
    }

    // Drive the incremental path: first highlight `s1` (no edit, populates the
    // retained root tree), drain it, then highlight `s2` with `edit`, draining
    // to collect the encoded event stream.
    fn incremental_events(
        ...
        // Pass 1: full parse of s1, fully drained to populate prev_root.
        {
            let it = hl
                .highlight_incremental(config, s1, &[], None, |_| None)
                .expect("incremental pass 1 starts");
            for e in it {
                e.expect("incremental pass 1 event");
            }
        }

        // Pass 2: incremental parse of s2 reusing the edited prev_root.
        let it = hl
            .highlight_incremental(config, s2, std::slice::from_ref(edit), None, |_| None)
            .expect("incremental pass 2 starts");
        it.map(|e| encode(&e.expect("incremental pass 2 event")))
            .collect()
    }
    ...
    fn incremental_insert_matches_full_reparse() {
        ...
        let s1 = "fn main() {\n    let x = 1;\n    println!(\"{}\", x);\n}\n";
        // Insert a new statement at the start of line 2 (right after the first
        // newline). Inserted text contains no newline, so the row is unchanged.
        let insert_at = s1.find('\n').expect("has a newline") + 1; // start of line 2
        let inserted = "    let y = 2;\n";
        let mut s2 = String::with_capacity(s1.len() + inserted.len());
        s2.push_str(&s1[..insert_at]);
        ...
        let edit = InputEdit {
            ...
            old_end_byte,
            new_end_byte,
            start_position: point_at(s1b, start_byte),
            old_end_position: point_at(s1b, old_end_byte),
            new_end_position: point_at(s2b, new_end_byte),
        };

        let full = full_events(&config, s2b);
        let incremental = incremental_events(&config, s1b, s2b, &edit);

        assert_eq!(
            incremental, full,
            "incremental (insert) event stream must equal full reparse"
        );
        // Sanity: a non-trivial stream actually got produced.
        assert!(full.len() > 4, "expected a non-trivial event stream");
    }
    ...
    fn incremental_delete_matches_full_reparse() {
        ...
        // s1 has an extra statement that s2 deletes, exercising
        // old_end_byte > new_end_byte (and old_end_position > start_position).
        let s1 = "fn main() {\n    let y = 2;\n    let x = 1;\n    println!(\"{}\", x);\n}\n";
        // Delete the whole `    let y = 2;\n` line (line 2).
        let del_start = s1.find('\n').expect("has a newline") + 1; // start of line 2
        let after = &s1[del_start..];
        let del_len = after.find('\n').expect("line 2 has a newline") + 1; // include the newline
        let del_end = del_start + del_len;

        let mut s2 = String::with_capacity(s1.len() - del_len);
        s2.push_str(&s1[..del_start]);
        s2.push_str(&s1[del_end..]);

        let s1b = s1.as_bytes();
        let s2b = s2.as_bytes();

        let start_byte = del_start;
        let old_end_byte = del_end;
        let new_end_byte = del_start; // nothing inserted
        let edit = InputEdit {
            start_byte,
            old_end_byte,
            new_end_byte,
        ...
        assert!(
            edit.old_end_byte > edit.new_end_byte,
            "delete case must have old_end > new_end"
        );
        ...
        assert_eq!(
            incremental, full,
            "incremental (delete) event stream must equal full reparse"
        );
        assert!(full.len() > 4, "expected a non-trivial event stream");
    }

    #[test]
    fn reset_forces_full_reparse_path() {
        // After `reset`, even with an edit supplied, the next call must fall back
        // to a full reparse (no stale tree reuse) and still match a full reparse.
        let config = build_config();
        let s1 = "fn a() { let z = 0; }\n";
        let s2 = "fn a() { let z = 42; }\n";
        let s1b = s1.as_bytes();
        let s2b = s2.as_bytes();

        // The "42" replaces "0": start at the byte of '0', old_end after '0',
        // new_end after "42".
        let start_byte = s1.find('0').expect("has 0");
        let old_end_byte = start_byte + 1; // length of "0"
        let new_end_byte = start_byte + 2; // length of "42"
        let edit = InputEdit {
            start_byte,
            old_end_byte,
            new_end_byte,
        ...
        let mut hl = Highlighter::new();
        // Populate, then reset, discarding the retained tree.
        {
            let it = hl
                .highlight_incremental(&config, s1b, &[], None, |_| None)
                .unwrap();
            for e in it {
                e.unwrap();
            }
        ...
        // With prev_root cleared, the edit is ignored (full reparse).
        let after_reset: Vec<_> = hl
            .highlight_incremental(&config, s2b, std::slice::from_ref(&edit), None, |_| None)
            .unwrap()
            .map(|e| encode(&e.unwrap()))
            .collect();

        assert_eq!(
            after_reset,
            full_events(&config, s2b),
            "post-reset stream must equal full reparse"
        );
    }
}
