...
fn decode_image_blocking(path: &Path) -> Result<ratatui_image::protocol::StatefulProtocol, String> {
    ...
    let dyn_img = image::ImageReader::new(std::io::Cursor::new(buf))
        .with_guessed_format()
...
// ── Keybinding hints (which-key style) ────────────────────────────────
...
fn pop_grapheme_returning(s: &mut String) -> bool {
    if s.is_empty() {
        return false;
    }
...
/// Render the keybinding hints overlay.
///
/// Lays out groups in a two-column flow, each group rendered as
/// `Title` followed by `Key  Label` rows. The current filter (if any)
/// narrows entries to those whose key or label contain the substring.
fn render_key_hints(area: Rect, buf: &mut Buffer, state: &KeyHintsState, theme: &Theme) {
    use crate::primitives::{Clear, Paragraph};
    ...
    let block = Block::default()
        ...
        .title(Line::from(title).style(Style::new().fg(theme.fg).bold()))
...
/// Render the image preview overlay.
fn render_image_preview(
    ...
    let block = Block::default()
        ...
        .border_type(BorderType::Rounded)
        .border_style(Style::new().fg(theme.fg_muted))
        .title(Line::from(title_text).style(Style::new().fg(theme.fg).bold()))
        .style(Style::new().bg(theme.bg));
    let inner = block.inner(popup);
    block.render(popup, buf);
...
/// Render the markdown preview overlay in a centered popup frame.
///
/// `tui-markdown::from_str` produces a ratatui `Text` with styled spans
/// (headings, bold, italic, code blocks, lists). We render it into the
/// popup's inner area honoring `state.scroll` for vertical scrolling.
