...
pub fn render_editor_pane(
    ...
    // No block here: the caller (`render_center`) wraps the entire
    // center column (tab strip + this pane) in one bordered Block, so
    // the editor's top border aligns with file_tree's and git_panel's
...
mod tests {
    ...
    #[test]
    fn render_editor_pane_does_not_draw_a_block_itself() {
        // The block belongs to `render_center` now — `render_editor_pane`
        // must draw nothing but content into the rect it was given.
        // Drawing a border here would double the one wrapped by the
        // caller and corrupt mouse hit-testing again.
        let area = Rect::new(0, 0, 20, 6);
        let mut buf = Buffer::empty(area);
        let mut viewport = ViewportState::default();
        let text_buf = TextBuffer::from_text("line\n".repeat(50).as_str());
    ...
    fn render_editor_pane_draws_secondary_cursor_with_accent_style() {
        // 20x3 inner-only rect: gutter + content, no border drawn here.
        // Secondary cursor sits at char column 2 of "alpha" → cell
        // column `gutter(2) + 2 = 4` on row 0.
